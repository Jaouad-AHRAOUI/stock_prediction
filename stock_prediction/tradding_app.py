...
def true_returns(start_date, end_date, dict_hard_data) :
    ...
    return df_close_prices, df_true_returns


def portfolio(close_price, true_returns, best_true, best_pred, cash_invest, true=False) :
    ...
    for df in best_ten :
        ...
        for stocks in df.index.to_list() :
            ...
            nb_stocks = amount_to_trade // close_price.loc[date_list[position -1], stocks]
            # the real amount invested in that stock
            amount_traded = nb_stocks * close_price.loc[date_list[position -1], stocks]
            ...
            perform_stock = (amount_end_of_day - amount_traded) * direction
            perf_daily_saved += perform_stock
            # we need the amount we would add if the return predicted was right
            predicted_return_amount = amount_traded * rets * direction
            ...
            dict_stocks[stocks][ptf_date] = [nb_stocks, rets, rets_true, predicted_return_amount, perform_stock]
